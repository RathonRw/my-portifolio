...
export function PhotoGallery({ photos }: PhotoGalleryProps) {
  ...
  const measureVisualOrder = useCallback(() => {
    ...
    if (positioned.length !== photos.length) {
      return;
    }
  ...
  useEffect(() => {
    ...
    if (didStartLightboxPrefetchRef.current) {
      return;
    }

    const requiredLoaded = Math.min(INITIAL_GALLERY_LOAD_TARGET, photos.length);
    if (loadedGalleryCount < requiredLoaded) {
      return;
    }
    ...
    const prefetchUrls = Array.from(
      new Set(
        photos.map((photo) => {
          const targetWidth =
            hasPhotoMetadata(photo) && viewportWidth >= 768
              ? Math.max(320, viewportWidth - 280)
              : viewportWidth;
          const optimizedWidth = closestNextImageWidth(targetWidth);
          return buildNextImageUrl(
            photo.url,
            optimizedWidth,
            LIGHTBOX_PREFETCH_QUALITY
          );
        })
      )
    );
    ...
    const run = async () => {
      for (const loc of toFetch) {
        ...
        fetch(
          ...
          console.error(
            "error while getting longitude and latitude for light box in image gallry"
          );
        });
  ...
  const handleGalleryImageLoad = useCallback(
    (photoId: string) => {
      ...
      if (loadedPhotoIdsRef.current.has(photoId)) {
        return;
      }
  ...
  const orderedPhotos = useMemo(() => {
    if (visualOrder.length !== photos.length) {
      return photos;
    }
  ...
  const handlePrev = useCallback(
    () =>
      setSelectedIndex((i) =>
        i === null
          ? null
          : (i - 1 + orderedPhotos.length) % orderedPhotos.length
      ),
          ...
          return (
            <button
              className={cn(
                "gallery-item",
                "animate-on-scroll",
                "mb-3 block w-full break-inside-avoid",
                "overflow-hidden rounded-lg",
                "cursor-pointer",
                "transition-all duration-300 ease-out",
                "hover:scale-[1.003] hover:brightness-[1.03]",
